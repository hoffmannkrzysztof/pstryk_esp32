  ...
  Verify signed firmware:
    ...
    if args.generate_key:
        ...
        if not args.pubkey:
            ...
            sys.exit(1)
        # LOCAL DEVIATION from upstream 3.3.8: upstream discards this return value,
        # so --verify exits 0 even when verification FAILS -- which silently turned
        # the release workflow's self-verify step into a decoration that `set -e`
        # could never catch. Exit non-zero so a bad signature fails the release.
        if not verify_signature(args.verify, args.pubkey, args.hash):
            sys.exit(1)
